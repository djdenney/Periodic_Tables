...
function hasMobileNumber(req, res, next) {
    ...
    if (mobileNumber) {
        ...
        next({
            status: 400,
            message: `${mobileNumber} is not a valid mobile number`
        })
    }
    return next({
        status: 400,
...
function validateMobileNumber(mobileNumber) {
    ...
    return numericMobileNumber.length === 10 && 
        numericMobileNumber.match(/^[0-9]+$/) != null
}

function validateReservationDate(reservationDate) {
    const todayParsedString = parseNumerals(new Date().toISOString().split('T')[0])
    const reservationDateParsedString = parseNumerals(reservationDate)        
    const isNotTuesday = toDate(reservationDate).toDateString().substring(0, 3) !== 'Tue'
    return reservationDateParsedString.length === 8 && 
...
function validateReservationTime(reservationTime, reservationDate) {
    ...
    return reservationDate >= openTime &&
...
async function validateStatus(req, res, next) {
    ...
    if (req.body.data.status !== 'unknown') {
        if (reservation[0].status === 'booked' || reservation[0].status === 'seated') {
            return next()
        }
        next({
            status: 400,
