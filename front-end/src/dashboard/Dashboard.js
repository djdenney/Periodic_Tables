...
function Dashboard({ today }) {
    ...
    function cancelButton(reservation) {
        if (reservation.status === "booked") {
            return (
                <button className="btn btn-danger" onClick={() => cancelReservation(reservation)}>
                    Cancel
                </button>
            )
        }
    ...
    async function cancelReservation(reservation) {
        try {
            if (window.confirm(`Do you want to cancel this reservation? This cannot be undone.`)) {
                const abortController = new AbortController()
                const response = await updateReservationStatus(reservation, 'cancelled', abortController.signal)
                history.go(0)
                return response
            }
        } catch (error) {
    ...
    const reservationRows = reservations.map((reservation) => {
        return (
            <tr className="text-truncate" key={reservation.reservation_id}>
                ...
                <td className="d-none d-md-table-cell" data-reservation-id-status={reservation.revervation_id}>{reservation.status}</td>
    ...
    const tableRows = tables.map((table) => (
        <tr className="text-truncate" key={table.table_id}>
            ...
            <td data-table-id-status={table.table_id}>{tableStatus(table.reservation_id)}</td>
            <td>
                {showFinishButton(table, table.reservation_id)}
            </td>
        </tr>
    ))
            ...
            <div className="container">
                <div className="d-grid gap-2 mb-2">
                    ...
                    <div className="btn-group">
                        ...
                        <button type="button" className="btn btn-outline-light" onClick={() => handleCurrent()}>Today</button>
                        <button type="button" className="btn btn-outline-light" onClick={() => handleNext()}>Next</button>
                    </div>
